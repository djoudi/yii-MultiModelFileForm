...
class MultiModelForm extends CWidget
{
    ...
    public $removeLinkWrapper = array(
        'tag' => 'span',
        'htmlOptions' => array('class'=>'remove-item'),
    );
     ...
     * @var array $options
     */
    // public $options = array('excludeSelector'=>'.remove-item');
     ...
     * @return boolean
     */
    public static function save($model, &$validatedItems, &$deleteItems = array(), $masterValues = array(), $fileAttributes = array())
    {
        ...
        if ($doValidate)
        {
            ...
            if (!self::validate($model, $validatedItems, $deleteItems, $masterValues, $formData, $fileAttributes))
                return false;
        }
        ...
        if (!empty($validatedItems)){
            foreach ($validatedItems as $item){
								...
								// cycle through all file attriubtes
								if(!empty($fileAttributes)){
									foreach($fileAttributes as $fileAttribute){
										if(is_object($item->$fileAttribute['attribute'])){
											
											$file = $item->$fileAttribute['attribute'];
										
											// if the destination save directory does not exist, create it
											if(!is_dir(Yii::getPathOfAlias('webroot') . '/' . $fileAttribute['destination'])){
												mkdir(Yii::getPathOfAlias('webroot') . '/' . $fileAttribute['destination'], 0777, true);
											}

											// replace unfriendly characters
											$uploadName = preg_replace('/[^A-Za-z0-9-_\.]/', '', $file->name);

											$pathInfo = pathinfo($file->name);
											$origName = $pathInfo['filename'];

											// check if file exists, if so append a number before the extension
											$fileCounter = 1;
											while(file_exists(Yii::getPathOfAlias('webroot') . '/' . $fileAttribute['destination'] . '/' . $uploadName)){
												$uploadName = $origName . '_' . $fileCounter++ . '.' . $pathInfo['extension'];
											}											
										
											$item->$fileAttribute['attribute']->saveAs(Yii::getPathOfAlias('webroot') . '/' . $fileAttribute['destination'] . '/' . $uploadName);
											$fileModel = new File;
											$fileModel->filename = $uploadName;
											$fileModel->path = $fileAttribute['destination'];
											$fileModel->save();
											$item->$fileAttribute['attribute'] = $fileModel->id;
										}
									}
								}
     ...
     * @return boolean
     */
    public static function validate($model, &$validatedItems, &$deleteItems = array(), $masterValues = array(), $formData = null, $fileAttributes = array())
    {
        ...
        if (!$widget->initItems($validatedItems, $deleteItems, $masterValues, $formData, $fileAttributes))
     ...
     */
    public function initItems(&$validatedItems, &$deleteItems, $masterValues = array(), $formData = null, $fileAttributes = array())
    {
        if (!isset($formData)){
        	$formData = $_POST;
				}
                ...
                $model->attributes = $attributes;
								...
								if(!empty($fileAttributes)){
									foreach($fileAttributes as $key => $value){
										$files[$value['attribute']] = CUploadedFile::getInstances($model, '[n__][' . $idx . ']' . $value['attribute']);
										if(!empty($files[$value['attribute']][$key])){
											$model->$value['attribute'] = $files[$value['attribute']][$key];
										}
									}											
								}
        ...
        if (isset($formData[$modelClass]['u__']))
        {
            foreach ($formData[$modelClass]['u__'] as $idx => $attributes)
            {
                ...
                $model->attributes = $attributes;
								...
								if(!empty($fileAttributes)){
									foreach($fileAttributes as $key => $value){
										$files[$value['attribute']] = CUploadedFile::getInstances($model, '[u__][' . $idx . ']' . $value['attribute']);
										if(!empty($files[$value['attribute']][$key])){
											$model->$value['attribute'] = $files[$value['attribute']][$key];
										}
									}											
        ...
        if(!empty($formData[$modelClass])) //has cloned elements
        {
            ...
            if (!empty($refArray))
                ...
                foreach ($refArray as $idx => $value)
                {
										...
										if(empty($value)){
											if(!empty($fileAttributes)){
												foreach($fileAttributes as $fileAttributeKey => $fileAttribute){
													$_fileAttribute = $fileAttributes[$fileAttributeKey]['attribute'];
													if(isset($_FILES[$modelClass]['name'][$_fileAttribute]) && $_FILES[$modelClass]['name'][$_fileAttribute][$idx] != ''){
														$value = $_FILES[$modelClass]['name'][$_fileAttribute][$idx];
													}													
												}												
											}									
										}
                    ...
                    $model->$refAttribute = $value;
										...
										$files = array();
										if(!empty($fileAttributes)){
											foreach($fileAttributes as $value){
												$files[$value['attribute']] = CUploadedFile::getInstances($model, $value['attribute']);
												if(!empty($files[$value['attribute']][$idx])){
													$model->$value['attribute'] = $files[$value['attribute']][$idx];
												}
											}											
										}
    ...
    public function getRemoveLink($isCopyTemplate=false)
    {
        ...
        $onClick = '$(this).parent().parent().remove(); mmfRecordCount--; alert(mmfStartCount); return false;';
    ...
    public function registerClientScript()
    {
        ...
        $cs->registerScript(__CLASS__ . '#' . $this->id, "jQuery('#{$this->id}').relCopy($options);jQuery('#{$this->id}').click(function(){return false;});");
    ...
    public function run()
    {
        ...
        if ($this->isSortable()){
            ...
            if (!$this->tableView){
            	echo CHtml::openTag('div', array('id' => $this->getSortSelectorId(), 'class' => $this->getSortSelectorId()));
						}
        }
...
class MultiModelRenderForm extends CForm
{
        ...
        if($this->isCopyTemplate)
        {
           ...
           if($this->parentWidget->hideCopyTemplate)
               $htmlOptions['style'] = !empty($htmlOptions['style'])? $htmlOptions['style'] . 'display:none;' : 'display:none;';
				...
				if($this->parentWidget->isSortable()){
					$content = '<div class="view-inner"><div class="handle"></div>' . $content . '</div>';
				}
    ...
    protected function renderFormElements()
    {
        ...
        $elements = $this->getElements();
				...
				// check how many form elements will be rendered (for use with file fields)
				$elementCount = 0;
				foreach($this->parentWidget->formConfig['elements'] as $element){
                    ...
                    if ($element->type == 'hidden'){
                        $output .= $element->render();
										...
										// file field
											...
											$elemOutput .= '<div class="file"><div class="left">' . Yii::app()->thumb->display($this->model, $elemName, 100, 100) . '</div>' . $this->model->file->filename . '</div>';
											if($elementCount > 1){
												$elemOutput .= '<a href="" class="file-remove" data-file-input-name="' . get_class($this->model) . '[' . $prefix . '][' . $this->index . '][' . $elemName . ']">remove file</a>';
											}	
											
											$elemOutput .= '<input type="hidden" name="' . get_class($this->model) . '[' . $prefix . '][' . $this->index . '][' . $elemName . ']" value="' . $this->model->$elemName . '" />';
    ...
    public function render()
    {
        ...
        $wrapperClass = $this->parentWidget->fieldsetWrapper['htmlOptions']['class'];
				...
				// assign even / odd classnames
				$interval = $this->index % 2 == 0 ? 'even' : 'odd';
